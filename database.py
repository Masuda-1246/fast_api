...
async def db_signup(data:dict) -> dict:
  ...
  if overlap_user:
    raise HTTPException(status_code=400, detail='Email is alraredy token')
  if not password or len(password) < 6:
    raise HTTPException(status_code=400, detail='Password too short')
  user = await collection_user.insert_one({"email":email, "password": auth.genereate_hashed_pw(password)})
...
async def db_login(data: dict) -> str:
  ...
  if not user or not auth.verify_pw(password, user["password"]):
    raise HTTPException(status_code=401, detail='Invlid email or password')
